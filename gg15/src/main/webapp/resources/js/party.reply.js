$(document).ready(function(){
	...
	var rowCount;
	//댓글 목록
	function selectData(pageNum,par_num){
		currentPage = pageNum;
		...
		$.ajax({
			type:'post',
			data:{pageNum:pageNum,par_num:par_num},
			url:'listReply.do',
			dataType:'json',
			cache:false,
			timeout:30000,
			success:function(data){
				...
				if(count < 0 || list == null){
					...
					$(list).each(function(index,item){
						...
						output += item.pop_date;
						if($('#mem_num').val()==item.mem_num){
							//로그인 한 회원 번호가 댓글 작성자 번호와 같으면
							output += '  <input type="button" data-num="'+item.pop_num+'" data-mem="'+item.mem_num+'" value="수정" class="modify-btn">';
							output += '  <input type="button" data-num="'+item.pop_num+'" data-mem="'+item.mem_num+'" value="삭제" class="delete-btn">';
						}
	...
	$('.paging-button input').click(function(){
		...
		selectData(pageNum,$('#par_num').val());
	});
	...
	$(document).on('click','.modify-btn',function(){
		...
		//댓글 글번호
		var pop_num = $(this).attr('data-num');
		//작성자 아이디
		...
		var modifyUI = '<form id="mre_form">'
			modifyUI += '   <input type="hidden" name="pop_num" id="mpop_num" value="'+pop_num+'">';
			modifyUI += '   <input type="hidden" name="mem_num" id="mmem_num" value="'+mem_num+'">';
			modifyUI += '   <textarea rows="3" cols="50" name="pop_content" id="mpop_content" class="rep-content">'+content+'</textarea>';
		...
		//입력한 글자수 셋팅
		var inputLength = $('#mpop_content').val().length;
		var remain = 300 - inputLength;
		remain += '/300';
		...
		$.ajax({
			...
			success:function(data){
				if(data.result=='logout'){
					...
					//$('#mre_form').parent().find('p').html($('#mpop_content').replace(/\n/g,'<br>'));
					$('#mre_form').parent().find('p').html($('#mpop_content').val().replace(/</g,'&lt;').replace(/>/g,'&gt;'));
					//수정폼 초기화
					initModifyForm();
	...
	$(document).on('click','.delete-btn',function(){
		...
		$.ajax({
			type:'post',
			url:'deleteReply.do',
			data:{pop_num:pop_num,mem_num:mem_num},
			dataType:'json',
			cache:false,
			timeout:30000,
			success:function(data){
