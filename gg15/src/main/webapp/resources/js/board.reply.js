$(document).ready(function(){
	...
	$(document).on('click','.modify-btn',function(){
		...
		//댓글 글번호
		var bor_num = $(this).attr('data-num');
		//작성자 아이디
		...
		var modifyUI = '<form id="mre_form">'
			modifyUI += '   <input type="hidden" name="bor_num" id="mbor_num" value="'+bor_num+'">';
			modifyUI += '   <input type="hidden" name="mem_num" id="mmem_num" value="'+mem_num+'">';
			modifyUI += '   <textarea rows="3" cols="50" name="bor_content" id="mbor_content" class="rep-content">'+content+'</textarea>';
		...
		//입력한 글자수 셋팅
		var inputLength = $('#mbor_content').val().length;
		var remain = 300 - inputLength;
		remain += '/300';
		...
		$.ajax({
			...
			success:function(data){
				if(data.result=='logout'){
					...
					//$('#mre_form').parent().find('p').html($('#mbor_content').replace(/\n/g,'<br>'));
					$('#mre_form').parent().find('p').html($('#mbor_content').val().replace(/</g,'&lt;').replace(/>/g,'&gt;'));
					//수정폼 초기화
					initModifyForm();
	...
	$(document).on('click','.delete-btn',function(){
		...
		$.ajax({
			type:'post',
			url:'deleteReply.do',
			data:{bor_num:bor_num,mem_num:mem_num},
			dataType:'json',
			cache:false,
			timeout:30000,
			success:function(data){
